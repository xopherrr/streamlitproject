...
@st.cache_data
def load_dataset():
    ds = xr.open_dataset('cmems_mod_glo_phy-thetao_anfc_0.083deg_P1D-m_1772597659853.nc')
    #monthly
    sst_monthly = ds['thetao'].resample(time='1MS').mean().load()
    return sst_monthly

# 2 plot function
def plot_monthly(data_bulan, nama_bulan, vmin, vmax):
    ...
    ax.set_extent([116, 120, -6, 2])

    gl = ax.gridlines(draw_labels=True, linestyle='--', alpha=0.3)
    gl.top_labels = False
    gl.right_labels = False

    plt.colorbar(p, orientation='horizontal', pad=0.08, label='SST (°C)')
    plt.title(f'SST Bulanan - {nama_bulan}', fontsize=15)
...
fig_hasil = plot_monthly(data_bulan, month_names[month_idx-1], vmin, vmax)
